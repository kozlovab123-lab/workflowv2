...
def _decrypt_api_key_with_fernet(encrypted_api_key: str, settings_service: SettingsService | None) -> str:
...
def decrypt_api_key(
    ...
    if result and not result.startswith("gAAAAA") and result != encrypted_api_key:
        return result

    # LFX auth stub returns Fernet ciphertext unchanged; decrypt with SECRET_KEY directly.
    try:
        return _decrypt_api_key_with_fernet(encrypted_api_key, settings_service)
    except Exception:
        logger.warning(
            "API key decryption failed after auth service and Fernet fallback. "
            "Check LANGFLOW_SECRET_KEY / secret_key file matches the key used to encrypt credentials."
        )
        return ""
