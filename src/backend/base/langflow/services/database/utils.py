...
def parse_uuid(value: UUID | str, *, field_name: str = "value") -> UUID:
    ...
    Accepts dashed UUID strings and 32-character hex strings (SQLite storage form).
...
async def session_get_by_pk(session: AsyncSession, model: type, pk: UUID | str) -> object | None:
    """Load a row by primary key, accepting dashed and 32-char hex UUID strings."""
    pk_uuid = parse_uuid(pk) if not isinstance(pk, UUID) else pk
    return await session.get(model, pk_uuid)
