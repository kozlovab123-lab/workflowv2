...
async def build_graph_from_data(flow_id: uuid.UUID | str, payload: dict, **kwargs):
    ...
    if "flow_name" not in kwargs:
        flow_name = await _get_flow_name(parse_uuid(flow_id, field_name="flow_id"))
    else:
        flow_name = kwargs["flow_name"]
    str_flow_id = str(flow_id)
    session_id = normalize_session_id(kwargs.get("session_id") or str_flow_id) or str_flow_id
    ...
    for vertex_id in graph.has_session_id_vertices:
        ...
        if vertex is None:
            ...
            raise ValueError(msg)
        # Always sync session_id from the current run so Chat Input/Output and Agent
        # memory retrieval use the same value (stale vertex params caused empty history).
...
async def build_graph_from_db_no_cache(flow_id: uuid.UUID, session: AsyncSession, **kwargs):
    """Build and cache the graph."""
    flow: Flow | None = await session_get_by_pk(session, Flow, flow_id)
