...
from lfx.log.logger import configure, logger, resolve_console_log_level
...
def _load_default_dotenv_files() -> Path | None:
    """Load user config .env when ``langflow run`` is invoked without ``--env-file``."""
    loaded: Path | None = None
    home_env = Path.home() / ".langflow" / ".env"
    if home_env.is_file():
        load_dotenv(home_env, override=False)
        loaded = home_env
    config_dir = os.environ.get("LANGFLOW_CONFIG_DIR")
    if config_dir:
        config_env = Path(config_dir) / ".env"
...
def run(
    *,
    ...
    log_level: str | None = typer.Option(
        None,
        help="Logging level. One of: [debug, info, warning, error, critical]. Defaults to error.",
        show_default=False,
    ),
    ...
    if env_file:
        ...
        env_file = _load_default_dotenv_files()

    # Set and normalize log level, with precedence: cli > env > default
    log_level = (log_level or os.environ.get("LANGFLOW_LOG_LEVEL") or "error").lower()
    os.environ["LANGFLOW_LOG_LEVEL"] = log_level

    if log_file is None:
        env_log = os.environ.get("LANGFLOW_LOG_FILE", "").strip()
        if env_log:
            log_file = Path(env_log)

    console_log_level = resolve_console_log_level(log_level)
    configure(log_level=console_log_level, log_file=log_file, log_rotation=log_rotation)

    # Create progress indicator (show verbose timing only when console is DEBUG)
    verbose = console_log_level == "debug"
    ...
    if platform.system() == "Windows":
        ...
        uvicorn.run(
            ...
            host=host,
            port=port,
            log_level=resolve_console_log_level(log_level),
            reload=False,
        ...
        with progress.step(6):
            ...
            options = {
                ...
                "log_level": log_level.lower() if log_level is not None else "error",
