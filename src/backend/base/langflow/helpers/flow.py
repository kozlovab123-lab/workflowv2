...
async def list_flows(*, user_id: str | None = None) -> list[Data]:
    ...
    try:
        async with session_scope() as session:
            uuid_user_id = parse_uuid(user_id, field_name="user_id") if isinstance(user_id, str) else user_id
...
async def list_flows_by_flow_folder(
    *,
    ...
    try:
        async with session_scope() as session:
            uuid_user_id = parse_uuid(user_id, field_name="user_id") if isinstance(user_id, str) else user_id
            uuid_flow_id = parse_uuid(flow_id, field_name="flow_id") if isinstance(flow_id, str) else flow_id
...
async def list_flows_by_folder_id(
    *, user_id: str | None = None, folder_id: str | None = None, order_params: dict | None = None
    ...
    try:
        async with session_scope() as session:
            uuid_user_id = parse_uuid(user_id, field_name="user_id") if isinstance(user_id, str) else user_id
            uuid_folder_id = parse_uuid(folder_id, field_name="folder_id") if isinstance(folder_id, str) else folder_id
            stmt = (
                select(Flow.id, Flow.name, Flow.updated_at)
                .where(Flow.user_id == uuid_user_id)
...
async def get_flow_by_id_or_name(
    *,
    ...
    try:
        async with session_scope() as session:
            uuid_user_id = parse_uuid(user_id, field_name="user_id") if isinstance(user_id, str) else user_id  # type: ignore[assignment]
            uuid_flow_id_or_name = val  # type: ignore[assignment]
            if isinstance(val, str) and attr == "id":
                uuid_flow_id_or_name = parse_uuid(val, field_name="flow_id")  # type: ignore[assignment]
...
async def load_flow(
    ...
    async with session_scope() as session:
        flow_pk = parse_uuid(flow_id, field_name="flow_id") if isinstance(flow_id, str) else flow_id
        graph_data = flow.data if (flow := await session_get_by_pk(session, Flow, flow_pk)) else None
    if not graph_data:
...
async def run_flow(
    ...
    if run_id:
        graph.set_run_id(parse_uuid(run_id, field_name="run_id"))
    if session_id:
