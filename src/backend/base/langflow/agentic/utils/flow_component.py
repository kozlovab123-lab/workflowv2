...
async def update_component_field_value(
    ...
    try:
        ...
        async with session_scope() as session:
            ...
            flow_pk = parse_flow_id(flow_id_str)
            if flow_pk is None:
                return {"error": f"Flow {flow_id_str} is not a valid UUID", "success": False}
            from langflow.services.database.utils import session_get_by_pk

            db_flow = await session_get_by_pk(session, Flow, flow_pk)
