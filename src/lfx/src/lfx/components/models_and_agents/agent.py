...
class AgentComponent(ToolCallingAgentComponent):
    ...
    def _resolve_selected_model(self):
        ...
        provider_mapping = get_provider_param_mapping(legacy_provider)
        return [
            {
                "name": legacy_model_name,
                "provider": legacy_provider,
                "metadata": {
                    "model_class": provider_mapping.get("model_class"),
                    "model_name_param": provider_mapping.get("model_param", "model"),
                    "api_key_param": provider_mapping.get("api_key_param", "api_key"),
                },
            }
        ]
    ...
    async def get_memory_data(self):
        ...
        current_id = self.input_value.get_id() if hasattr(self.input_value, "get_id") else getattr(self.input_value, "id", None)
        if current_id:
            return [message for message in messages if message.get_id() != current_id]
