...
def resolve_console_log_level(level: str | None = None) -> str:
    """Log level for terminal/uvicorn (quiet by default when writing debug to a file)."""
    level = (level or os.getenv("LANGFLOW_LOG_LEVEL", "error")).strip().lower()
    explicit = os.getenv("LANGFLOW_CONSOLE_LOG_LEVEL", "").strip().lower()
    if explicit:
        return explicit
    if os.getenv("LANGFLOW_LOG_FILE", "").strip() and level == "debug":
        return "info"
    return level


def resolve_file_log_level(level: str | None = None) -> str:
    """Log level for LANGFLOW_LOG_FILE (defaults to LANGFLOW_LOG_LEVEL)."""
    explicit = os.getenv("LANGFLOW_LOG_FILE_LEVEL", "").strip().lower()
    if explicit:
        return explicit
    return (level or os.getenv("LANGFLOW_LOG_LEVEL", "error")).strip().lower()
...
def configure(
    *,
    ...
    env_level = os.getenv("LANGFLOW_LOG_LEVEL", "ERROR")
    console_level_str = resolve_console_log_level(log_level or env_level)
    file_level_str = resolve_file_log_level(env_level)

    requested_min_level = LOG_LEVEL_MAP.get(console_level_str.upper(), logging.ERROR)
    if current_min_level == requested_min_level:
        return

    log_level = console_level_str
    ...
    # Console uses console_level_str; file handler may use a more verbose level
    numeric_console = LOG_LEVEL_MAP.get(console_level_str.upper(), logging.ERROR)
    numeric_file = LOG_LEVEL_MAP.get(file_level_str.upper(), logging.ERROR)

    # Create wrapper class and attach the min level for later comparison
    wrapper_class = structlog.make_filtering_bound_logger(numeric_console)
    wrapper_class.min_level = numeric_console
    ...
    if log_file:
        ...
        logging.root.setLevel(min(numeric_console, numeric_file))
