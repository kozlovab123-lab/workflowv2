...
MODEL_PROVIDER_METADATA: dict[str, Any] = {
    ...
    "SBER": {
        ...
        "variables": [
            {
                "variable_name": "GigaChat credentials",
                "variable_key": "GIGACHAT_CREDENTIALS",
                "required": True,
                "is_secret": True,
                "is_list": False,
                "options": [],
                "langchain_param": "credentials",
                "component_metadata": {
                    "mapping_field": "credentials",
                    "required": False,
                    "advanced": True,
                    "info": "Falls back to GIGACHAT_CREDENTIALS environment variable",
                },
            }
        ],
        "api_docs_url": "https://developers.sber.ru/docs/ru/gigachat/api/overview",
        "mapping": {
            "model_class": "GigaChat",
            "model_param": "model",
            "api_key_param": "credentials",
...
def get_provider_param_mapping(provider: str) -> dict[str, str]:
    ...
    for var in metadata.get("variables", []):
        ...
        if mapping_field and langchain_param:
            ...
            if "api_key" in mapping_field or "credentials" in mapping_field:
