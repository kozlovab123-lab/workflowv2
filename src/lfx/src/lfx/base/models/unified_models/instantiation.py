...
def get_llm(
    ...
    *,
    ...
    api_key_param = metadata.get("api_key_param")
    model_name_param = metadata.get("model_name_param")
    if provider and (not api_key_param or not model_name_param):
        from lfx.base.models.model_metadata import get_provider_param_mapping

        provider_mapping = get_provider_param_mapping(provider)
        ...
        api_key_param = api_key_param or "api_key"
        model_name_param = model_name_param or "model"
    ...
    api_key = unified_models_module.ensure_decrypted_credential(api_key)
    ...
    if provider in {"IBM WatsonX", "IBM watsonx.ai"}:
        ...
        # GigaChat API often uses corporate/self-signed TLS chains on Windows.
