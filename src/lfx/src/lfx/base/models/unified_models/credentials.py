...
from lfx.utils.session_utils import coerce_uuid
...
def ensure_decrypted_credential(value: str | None) -> str | None:
    """Return a plaintext credential; decrypt Fernet blobs from the DB if needed."""
    if not value or not isinstance(value, str):
        return value
    stripped = value.strip()
    if not stripped:
        return None
    if not stripped.startswith("gAAAAA"):
        return stripped
    try:
        from langflow.services.auth.utils import decrypt_api_key
        from langflow.services.deps import get_settings_service

        decrypted = decrypt_api_key(stripped, get_settings_service())
    except Exception:  # noqa: BLE001
        return None
    if not decrypted or not str(decrypted).strip():
        return None
    decrypted = str(decrypted).strip()
...
def get_api_key_for_provider(user_id: UUID | str | None, provider: str, api_key: str | None = None) -> str | None:
    ...
    def _resolve_var_name(var_name: str) -> str | None:
        ...
        if user_id and not (isinstance(user_id, str) and user_id == "None"):
            ...
            async def _get_by_var_name():
                async with session_scope() as session:
                    ...
                    if variable_service is None:
                        return None
                    try:
                        user_pk = coerce_uuid(user_id) if isinstance(user_id, str) else user_id
                        if user_pk is None:
                            return None
            ...
            if value and str(value).strip():
                return ensure_decrypted_credential(str(value).strip())
        return None
    ...
    if api_key:
        return ensure_decrypted_credential(api_key)

    env_key = os.getenv(variable_name)
    return ensure_decrypted_credential(env_key)
...
def get_all_variables_for_provider(user_id: UUID | str | None, provider: str) -> dict[str, str]:
    ...
    async def _get_all_variables():
        async with session_scope() as session:
            ...
            values = {}
            user_id_uuid = coerce_uuid(user_id) if isinstance(user_id, str) else user_id
