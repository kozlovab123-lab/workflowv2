...
from lfx.memory import aget_messages, delete_message
...
class LCAgentComponent(Component):
    ...
    def _data_to_messages_skip_empty(self, data: list[Data]) -> list[BaseMessage]:
        ...
        for value in data:
            ...
            if text is None and hasattr(value, "data") and isinstance(value.data, dict):
    ...
    def _chat_history_to_lc_messages(self, chat_history: list) -> list[BaseMessage]:
        """Convert stored chat history into LangChain messages for the agent."""
        if not chat_history:
            return []

        if isinstance(chat_history, Data):
            return self._data_to_messages_skip_empty([chat_history])

        if isinstance(chat_history, Message):
            return self._data_to_messages_skip_empty([chat_history])

        if isinstance(chat_history, list):
            if all(isinstance(m, Message) for m in chat_history):
                return self._data_to_messages_skip_empty(chat_history)
            if all(hasattr(m, "to_lc_message") and callable(m.to_lc_message) for m in chat_history):
                return self._data_to_messages_skip_empty(chat_history)
            if all(hasattr(m, "to_data") and callable(m.to_data) for m in chat_history):
                return self._data_to_messages_skip_empty([m.to_data() for m in chat_history])

        return []

    def _get_effective_session_id(self) -> str | None:
        if not hasattr(self, "graph"):
            return None
        session_id = normalize_session_id(getattr(self.graph, "session_id", None))
        if session_id:
            return session_id
        return normalize_session_id(getattr(self.graph, "flow_id", None))

    def _get_current_input_message_id(self) -> str | None:
        input_value = getattr(self, "input_value", None)
        if input_value is None:
            return None
        if hasattr(input_value, "get_id"):
            return input_value.get_id()
        return getattr(input_value, "id", None)

    def _format_history_for_system_prompt(self, history_messages: list[BaseMessage]) -> str:
        lines: list[str] = []
        ...
        if not lines:
            return ""
        return (
            "История текущего диалога (используй для ответа; имя и другие факты из этого чата "
            "не являются внешними персональными данными):\n" + "\n".join(lines)
        )

    def _inject_history_into_input(self, input_dict: dict, history_text: str) -> None:
        current_input = input_dict.get("input", "")
        if not history_text or not isinstance(current_input, str) or not current_input.strip():
            return
        input_dict["input"] = (
            "История текущего диалога:\n"
            f"{history_text}\n\n"
            "Ответь на текущий вопрос пользователя, опираясь на историю выше.\n"
            f"Текущий вопрос: {current_input}"
        )

    async def _resolve_chat_history(self) -> list[BaseMessage]:
        """Load chat history from DB (preferred) or component state for agent context."""
        session_id = self._get_effective_session_id()
        if session_id:
            limit = getattr(self, "n_messages", 100) or 100
            stored_messages = await aget_messages(
                session_id=session_id,
                order="ASC",
                limit=limit,
            )
            current_id = self._get_current_input_message_id()
            if current_id:
                stored_messages = [m for m in stored_messages if m.get_id() != current_id]
            history_messages = self._chat_history_to_lc_messages(stored_messages)
            if history_messages:
                return history_messages

        if hasattr(self, "chat_history") and self.chat_history:
            return self._chat_history_to_lc_messages(self.chat_history)
    ...
    async def run_agent(
        ...
        history_messages = await self._resolve_chat_history()
        await logger.adebug(f"Agent chat history messages for prompt: {len(history_messages)}")
        if history_messages:
            input_dict["chat_history"] = history_messages
            history_text = self._format_history_for_system_prompt(history_messages)
            base_prompt = system_prompt_to_use or ""
            memory_instruction = (
                "Ты продолжаешь текущий диалог. Сведения, которые пользователь уже сообщил "
                "в этом чате (включая имя), — часть разговора. Используй историю для ответов."
            )
            if history_text:
                input_dict["system_prompt"] = f"{base_prompt}\n\n{memory_instruction}\n\n{history_text}".strip()
                self._inject_history_into_input(input_dict, history_text)
            elif base_prompt.strip():
                input_dict["system_prompt"] = base_prompt
        elif system_prompt_to_use and system_prompt_to_use.strip():
