    ...
    async def build(
        ...
        **kwargs,
        ...
        async with self.lock:
            ...
            if inputs is not None and "session" in inputs and inputs["session"] is not None:
                session_id = normalize_session_id(inputs["session"]) or inputs["session"]
                if self.graph is not None:
                    self.graph.session_id = session_id
                if self.has_session_id:
                    self.update_raw_params({"session_id": session_id}, overwrite=True)
