...
class Graph:
    ...
    async def arun(
        ...
        *,
        ...
        if session_id:
            ...
            self.session_id = normalize_session_id(session_id) or session_id
    ...
    async def build_vertex(
        ...
        *,
        ...
        """
        if inputs_dict and inputs_dict.get("session"):
            from lfx.utils.session_utils import normalize_session_id

            session_id = normalize_session_id(inputs_dict["session"]) or inputs_dict["session"]
            self.session_id = session_id
            for vtx_id in self.has_session_id_vertices:
                vtx = self.get_vertex(vtx_id)
                if vtx is not None:
                    vtx.update_raw_params({"session_id": session_id}, overwrite=True)
